...
module keyboard_decoder (
    ...
    // Internal control codes placed in the existing 8-bit keyboard FIFO.
    // They are outside the printable ASCII range and do not conflict with
    // Enter (0Ah) or Backspace (08h).
    localparam [7:0] KEY_LEFT  = 8'h11;
    localparam [7:0] KEY_RIGHT = 8'h12;
    localparam [7:0] KEY_UP    = 8'h13;
    localparam [7:0] KEY_DOWN  = 8'h14;
        ...
        if (reset) begin
            ...
            if (scan_valid && scan_ready) begin
                if (scan_code == 8'hE0) begin
                    ...
                    // PS/2 Set-2 direction-key make codes are prefixed by E0.
                    // Release sequences (E0 F0 xx) have already been handled
                    // by the break_pending branch above and emit no event.
                    case (scan_code)
                        8'h6B: begin // Left
                            event_ascii     <= KEY_LEFT;
                            event_enter     <= 1'b0;
                            event_backspace <= 1'b0;
                            event_valid     <= 1'b1;
                        end
                        8'h74: begin // Right
                            event_ascii     <= KEY_RIGHT;
                            event_enter     <= 1'b0;
                            event_backspace <= 1'b0;
                            event_valid     <= 1'b1;
                        end
                        ...
                        8'h72: begin // Down
                            event_ascii     <= KEY_DOWN;
                            event_enter     <= 1'b0;
                            event_backspace <= 1'b0;
                            event_valid     <= 1'b1;
                        end
                        default: begin
                            // Consume unsupported extended make codes without
                            // exposing them as ordinary ASCII characters.
                        end
                    endcase
                end
