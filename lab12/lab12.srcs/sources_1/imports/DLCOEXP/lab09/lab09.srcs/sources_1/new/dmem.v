...
module dmem #(
    // The 32-bit little-endian data image is split into four byte-lane files:
    // lane 0 contains bits [7:0], lane 1 contains bits [15:8], and so on.
    // Empty filenames preserve the original uninitialized-memory behavior.
    ...
    initial begin
        if (INIT_FILE0 != "")
            $readmemh(INIT_FILE0, ram0);
        if (INIT_FILE1 != "")
            $readmemh(INIT_FILE1, ram1);
        if (INIT_FILE2 != "")
            $readmemh(INIT_FILE2, ram2);
        if (INIT_FILE3 != "")
            $readmemh(INIT_FILE3, ram3);
    end
